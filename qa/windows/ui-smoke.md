...
Run this only on Windows from a packaged app image with its VLC and FFmpeg dependencies present. It checks native integration that the deterministic `ui-flow` Maven tests intentionally fake.
...
2. **Import the fixture** — Choose *Import Match*, use the native chooser to select `ui-smoke.mp4`, and confirm Rallies opens with the project/source path. Capture a screenshot.
3. **VLC playback** — Play, pause, and seek within the short clip. Preview state and timeline position should change without an error dialog. Capture a screenshot.
4. **Edit and score** — In Rallies mark one point boundary pair; set Colors brightness to 20; set Crop rotation to 15 degrees; then award Player 1 a point in Scoring. The visible values and saved project data should retain each edit. Capture a screenshot.
